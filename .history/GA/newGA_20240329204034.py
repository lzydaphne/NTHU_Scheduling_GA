...
def calculate_makespan(chromosome, processing_times, setup_times, num_machines):
    ...
    last_family = [
        -1
    ] * num_machines  # To track the last family processed by each machine
    ...
    for job in chromosome:
        ...
        setup_time = (
            0
            if last_family[machine_index] == setup_times[job][1]
            else setup_times[job][0]
        )
...
def fitness(chromosome, processing_times, setup_times, num_machines):
    ...
    makespan = calculate_makespan(
        chromosome, processing_times, setup_times, num_machines
    )
...
instance_setup_times = [
    mock_setup_times[family] for family in data[data["instance"] == "in1"]["family"]
]
...
chromosome_fitness = fitness(
    example_chromosome, instance_processing_times, instance_setup_times, num_machines
)
...
def run_generation(population, fitness_func, tournament_size, num_survivors):
    ...
    fitness_scores = [
        fitness_func(
            individual, instance_processing_times, instance_setup_times, num_machines
        )
    ...
    new_population = survivor_selection(
        ...
        fitness_scores
        + [
            fitness(ind, instance_processing_times, instance_setup_times, num_machines)
...
num_survivors = len(
    initial_population
)  # The number of survivors to keep for the next generation

# Running a single generation
new_population = run_generation(
    initial_population, fitness, tournament_size, num_survivors
)
